...
export default function PostForm({ post }) {
    ...
    const submit = async (data) => {
        if (post) {
            ...
            if (file) {
                DataService.deleteFile(post.image_id);
            }
            ...
            if (file) {
                ...
                if(userdata.userdata){
                    Authorname=userdata.userdata.email
                }
                else{
                    Authorname=userdata.email
                }
                const dbPost = await DataService.createPost({ ...data, user_id: userdata.$id,authorname:Authorname});
                if (dbPost) {
                    navigate(`/post/${dbPost.$id}`);
                }
            }
    ...
    return (
        <form onSubmit={handleSubmit(submit)} className="flex flex-wrap">
            ...
            <div className="w-1/3 px-2">
                ...
                />
                <Button type="submit" bgColor={post ? "bg-green-500" : undefined} className="w-full font-semibold">
                    {post ? "Update" : "Submit"}
                </Button>
            </div>
        </form>
